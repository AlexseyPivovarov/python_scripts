...
class CinemaApi(Cinema):
    ...
    def set_film_to_seans(self, hall_index: int, film_index_from_poster: int, hours: int, minutes: int = 0):
        self.halls[hall_index].set_film_to_seans(self.poster[film_index_from_poster], hours * 60 + minutes)

    def set_ticket(self, hall_index: int, film_index: int, seat_number: int):
        self.halls[hall_index].time_scale[film_index].seats[seat_number].set_reserved()

    def get_poster_info(self):
        return "; ".join([item.get_info() for item in self.poster])

    def get_time_scale_info(self, hall_index: int):
        return self.halls[hall_index].get_info()

    def get_seans_seats_info(self, hall_index: int, film_index: int):
        return self.halls[hall_index].time_scale[film_index].get_seats_info()
