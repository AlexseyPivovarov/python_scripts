...
class Check:
    ...
    def get_info(self):
        ...
        for item in self.buy:
            space1 = PAPER - len(item.product.unit_name) - len(str(item.product.unit_weight)) - \
                     len(str(item.product.unit_coast)) - 2
            space2 = PAPER - len(str(item.quantity)) - len(str(item.weight)) - len(str(item.coast)) - 3
            print("{0} {1}{2:<{4}}${3}\nx{5} {6}{2:<{8}}${7}\n"
                  "".format(item.product.unit_name, item.product.unit_weight, item.product.units,
                            item.product.unit_coast, space1, item.quantity, item.weight, item.coast, space2))
